...
2. Reduce FASTQ files to a sensible depth (default --depth 100)
  - _These two steps are needed because there is an optimal read coverage level for accurate assembly; see today's genome assembly lecture for an explanation_
...
As with other Galaxy tools, you can inspect the contents of these files by clicking on the filename in the `History` sidebar, and also by clicking on the corresponding `eye` (`View`) icon to view the trimmed reads in the workspace.
...
::: { .callout-tip }
**Inspect the assembly graph by clicking on the `eye` (`View`) icon for the Assembly Graph Image output.**
:::
