...
::: { .callout-note }
Galaxy will advise that you use `Falco` instead of `FastQC`.

The two tools are essentially identical, but `Falco` is a more recent and faster implementation. If you choose to use `Falco` the process is very similar to using `FastQC`.
:::
...
::: { .callout-tip }
You **do not** have to wait for one job to finish before starting a new one.

Galaxy handles _scheduling_, and can perform more than one task at a time, so it is safe to queue up multiple jobs and set them running.
:::
...
When the run is complete, click on the `eye` (`View`) icon of the `Webpage` result to inspect the `FastQC` output.

::: { .callout-tip }
You can press and hold **CTRL** (on a PC) or **CMD ⌘** (macOS) when clicking the `eye` (`View`) icon to open the result in its own window.
:::
:::
...
::: { .column-margin }
![Example `FastQC` summary showing passes for most sections of the report including "Basic Statistics," warnings for some sections including "Overrepresented sequences," and a fail for "Sequence Duplication Levels"](assets/images/fastqc_summary.png){#fig-fastqc-summary fig-alt="Summary section of a FastQC report showing passes for most sections, warnings for some sections, and failure for a single section." width="80%" .lightbox}
:::
...
![Per base sequence quality score plot from `FastQC` indicating high quality sequencing reads. Quality score ranges are presented as a boxplot, with the mean drawn as a line connecting each boxplot.](assets/images/fastqc_perbase.png){#fig-fastqc-perbase width=80% .lightbox}
...
![Per sequence quality score plot from `FastQC` indicating overall high quality sequencing reads.](assets/images/fastqc_perseq.png){#fig-fastqc-perseq width=80% .lightbox}
