...
::: { .callout-note }
Galaxy will recommend that you use a tool called `Minibwa` instead of `BWA-MEM2`.

Both tools perform the same task, but `Minibwa` is faster. The tools do not necessarily give the same output, but you could use `Minibwa` here if you wanted to.
:::

1. Navigate to the `BWA-MEM2 - map` tool
2. Select the `BWA-MEM2 - map` tool
3. Choose `Use a reference from history and build index if necessary` under `Will you select a reference genome…`
4. Choose the `SARS_AY278741.fasta` sequence under `Use the following dataset as the reference sequence`
5. Choose `Paired` under `Single or Paired-end reads`
...
![`JBrowse` visualisation of a `BWA-MEM2` mapping of SARS-CoV-2 reads against a SARS 2003 reference genome. The annotation track shows predicted genes, including the spike (S) protein. The mapping track shows individual SNPs (teardrops), and total mapping read depth (the stacks above the teardrops).](assets/images/bwamem_mapping.png){#fig-bwamem-1 width="80%" .lightbox}
...
![`JBrowse` visualisation of a `BWA-MEM2` mapping of SARS-CoV-2 reads against a SARS 2003 reference genome in the regions of the spike (S) protein.](assets/images/bwamem_snps.png){#fig-bwamem-2 width="80%" .lightbox}
