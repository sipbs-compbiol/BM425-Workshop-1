...
2. R1-unpaired, R2-unpaired: these are single reads, where one member of the read pair was low quality and discarded
:::
...
You can inspect the contents of these files by clicking on the filename in the `History` sidebar, and also by clicking on the corresponding `eye` (`View`) icon to view the trimmed reads in the workspace.
